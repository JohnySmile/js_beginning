...
btn.forEach((button) => {
    button.addEventListener('click', clickedButtonEvent => this.handleClick(clickedButtonEvent)); 
});
...
function handleClick(clickedButtonEvent) {
    ...
    let buttonInnerText = cardNode.querySelector('button').innerText;
    
    if (buttonInnerText == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (buttonInnerText == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let loremText = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque luctus ipsum id ipsum tempor dapibus.';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.productName.insertAdjacentHTML('afterend', `<div class="desc"> ${loremText}</div>`);
